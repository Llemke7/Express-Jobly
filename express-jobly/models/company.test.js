...
describe("findAll with filters", function () {
  ...
  test("works with maxEmployees filter", async function () {
    const companies = await Company.findAll({ maxEmployees: 200 });
    expect(companies).toEqual([
      {
        handle: "c1",
        name: "C1",
  ...
  test("works with minEmployees and maxEmployees filters", async function () {
    const companies = await Company.findAll({ minEmployees: 2, maxEmployees: 3 });
    expect(companies).toEqual([
      {
        handle: "c2",
        name: "C2",
  ...
  test("returns 400 error with invalid filtering parameters", async function () {
    try {
      ...
      expect(err instanceof BadRequestError).toBeTruthy();
      expect(err.message).toContain("minEmployees cannot be greater than maxEmployees");
    }
  });
});
/************************************** get */
